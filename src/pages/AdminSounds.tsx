...
import { Copy, Upload, Pencil, Trash2, RefreshCw, Play, Pause, Music } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto max-w-6xl px-6 py-10">
        <h1 className="font-heading text-4xl text-gold mb-2">Sounds</h1>
        <p className="text-muted-foreground mb-8">Upload, name, replace, preview, and copy URLs of stored audio assets.</p>

        <Card className="p-6 mb-8 bg-card/50">
          <h2 className="font-heading text-xl text-gold mb-4">Upload new sound</h2>
        ...
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-heading text-xl text-gold">Stored sounds ({items.length})</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <Card key={item.name} className="overflow-hidden bg-card/50">
              <div className="aspect-video bg-muted/30 flex items-center justify-center overflow-hidden">
                <div className="flex flex-col items-center gap-3">
                  <Music className="h-12 w-12 text-bronze/60" />
                  <Button
                    size="sm"
                    variant="outline"
                    className="gap-2"
                    onClick={() => togglePlay(item.publicUrl)}
                  >
                    {playing === item.publicUrl ? (
                      <Pause className="h-4 w-4" />
                    ) : (
                      <Play className="h-4 w-4" />
                    )}
                    {playing === item.publicUrl ? "Pause" : "Preview"}
                  </Button>
                </div>
              </div>
          ...
          {!loading && items.length === 0 && (
            <div className="col-span-full text-center text-muted-foreground py-12">No sounds yet.</div>
          )}
        </div>
      </div>
    </div>
